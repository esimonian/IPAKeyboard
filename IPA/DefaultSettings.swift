...
class DefaultSettings: ExtraView, UITableViewDataSource, UITableViewDelegate {
    ...
    override var darkMode: Bool {
        didSet {
            self.updateAppearance(dark: darkMode)
        }
    }
    ...
    func loadNib() {
        ...
        self.updateAppearance(dark: self.darkMode)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as? DefaultSettingsTableViewCell {
            ...
            if cell.sw.allTargets.count == 0 {
                cell.sw.addTarget(self, action: Selector(("toggleSetting:")), for: UIControlEvents.valueChanged)
            }
    ...
    func updateAppearance(dark: Bool) {
        if dark {
            ...
            self.backButton?.setTitleColor(blueColor, for: UIControlState.normal)
