...
class KeyboardConnector: KeyboardKeyBackground {
    ...
    func generateConvertedPoints() {
        ...
        if let superview = self.superview {
            let startPoints = self.startConnectable!.attachmentPoints(direction: self.startDir)
            let endPoints = self.endConnectable!.attachmentPoints(direction: self.endDir)
    ...
    override func generatePointsForDrawing(_ bounds: CGRect) {
        ...
        //CGPathMoveToPoint(path, NULL, myConvertedStartPoints.0.x, myConvertedStartPoints.0.y)
